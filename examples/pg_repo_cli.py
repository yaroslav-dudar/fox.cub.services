...
async def fetch_report(match_repo: repository.MatchPgRepository,
                       min_value, max_value, event_ids, season_ids):
    start_at = time.time()
    await match_repo.create_multi_season_table()
    low_table_teams = await match_repo.get_by_score(event_ids, season_ids, 0.0, 1.0, 0.0, 3.1)
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(fetch_report(match_repo, 15, 20, [2], [4, 2396, 8668, 8863]))
